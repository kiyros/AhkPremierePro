...
totalWidth := SysGet(0)
totalHeight := SysGet(1)
position_image := "image_sources/position.png"
...
; #NoEnv ; recomended for performance and compat with future AHK releases.
; ; #Warn ;
; sendMode Input
...
; Click and hold to scale for premiere pro
^!s:: {
	MouseGetPos &xpos, &ypos
	scale_image := "image_sources/scale.png"

	; Image search
	; ImageSearch &foundX, &foundY, 0, 0, totalHeight, totalWidth, *50 position_image

	if ImageSearch(&x, &y, 0, 0, totalWidth, totalHeight, scale_image) {
		x := x + 135
		y := y + 10
...
; click and hold to poition x
^!x:: {
	MouseGetPos &xpos, &ypos

	if ImageSearch(&x, &y, 0, 0, totalWidth, totalHeight, position_image) {

		MouseMove x + 135, y + 10
		MouseClick "left", , , , , "D"
		MouseMove x + 1, y + 1 ; used to get the scrollable tooltip to showup before
	...
	MouseMove xpos, ypos
	; ToolTip "moved back to original position"

	return
}

; click and hold poition y
^!y:: {
	MouseGetPos &xpos, &ypos

	if ImageSearch(&x, &y, 0, 0, totalWidth, totalHeight, position_image) {

		MouseMove x + 175, y + 10
		MouseClick "left", , , , , "D"
		MouseMove x + 1, y + 1 ; used to get the scrollable tooltip to showup before
...
;     ; Calculate offset dynamically based on the located position of the button
;     scaleButtonX := CalculateScaleButtonX()
;     scaleButtonY := CalculateScaleButtonY()

;     MouseMove, scaleButtonX, scaleButtonY
;     Click, Down

;     KeyWait, s ; Wait for "s" key to be released

;     Click, Up
;     ; Remove unnecessary Sleep command
...
; ;click functions
; ^!s:: ; Click and hold scale for premiere pro
;     MouseGetPos, xpos, ypos
...
; F12:: ;  Broadcast Compressor
; 	preset("Broadcast Compressor")
; 	return
...
; ; for face cams
; !1:: ;cam location 1
; 	preset("BottomLeftZoom 500")
; 	return

; !0:: ;cam location 2
; 	preset("TopLeftZoom 400")
; 	return
...
preset(item)
{
	...
	sleep 70 ;<—Use this line if you don't want to use the loop below. But the loop should work perfectly fine as-is, without any modification from you.
	...
	; 	if (waiting2 > 40)
	; 		{
	; 		tooltip "FAIL - no caret found. If your cursor will not move hit the button to call the preset() function again.`nTo remove this tooltip refresh the script using its icon in the taskbar.`n`nIt's possible Premiere tried to AUTOSAVE at just the wrong moment!"
	...
	MouseMove xCaret, yCaret ;this moves the cursor, instantly, to the position of the caret.
	...
	ControlGetPos &XX, &YY, &Width, &Height, classNN
	; MsgBox classNN, yy
	...
	MouseMove XX-15, YY+30, 0 ;--------------------for 100% UI scaling, this moves the cursor onto the magnifying glass
	...
	MouseMove 41, 70, 0, "R" ;----------------------(for 100% UI)
	MouseGetPos &x, &y
	sleep 10
	...
	; ; ;sleep 50
	; ; ControlGetPos &xxx, &yyy, &www, &hhh, ControlGetClassNN, ahk_class %class%, SubWindow, SubWindow ;;-I tried to exclude subwindows but I don't think it works...?
	...
	MouseClickDrag "L", x, y, xposP, yposP, 100 ;---clicks the left button down, drags this effect to the cursor's pervious coordinates and releases the left mouse button, which should be above a clip, on the TIMELINE panel.
	sleep 5
	...
	; Blockinput false ;returning mouse movement ability
